...
a = df1.iloc[:, 0:2] #[:, = all rows,  0:2 = columns 1 and 3
print(a)
a1 = df1.loc[:, "name":"score"] #different method of doing it!

"""
(2) Select 'name' and 'qualify' columns in rows 1, 3, 5, 6 from the data frame.
...
b = df1.iloc[[1,3,5,6], [1,3]] #[:, = all rows,  0:2 = columns 1 and 3
print(b)
...
"""
c = df1[df1.iloc[:,2] >2] #print all rows [:] , but only apply the condition to the 2 index
print(c)
...
d = df1[(df1.iloc[:,1] >=15) & (df1.iloc[:,1] <=20)]
print(d)
...
"""
e = df1.sort_values(by =['attempts', 'name'])
print(e)
